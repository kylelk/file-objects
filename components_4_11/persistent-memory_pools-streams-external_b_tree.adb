...
package body Persistent.Memory_Pools.Streams.External_B_Tree is
...
--     end Dump;
   ...
   function Is_In (Container : B_Tree; Key : Byte_Index)
      return Boolean is
      Lock  : Holder (Container.Pool);
      Node  : Byte_Index;
      Index : Integer;
   ...
   procedure Add
      ...
      Lock : Holder (Container.Pool);
   begin
      Unchecked_Add (Container, Key, Value);
   end Add;
...
--        end if;
   ...
   function Find
      ...
      pragma Inline (Find);
      Derived : B_Tree'Class renames B_Tree'Class (Container);
   ...
   begin
      ...
      begin
         loop
            This := (From + To) / 2;
            declare
               Current : constant Byte_Index :=
                  Get
                  (  Keys,
                     Keys'First + (Block_Offset (This) - 1) * 8
                  );
            begin
               case Compare (Derived, Key, Current) is
                  when Same =>
                     return This;
                  when Before =>
                     if This - From <= 1 then
   ...
   function Get_Key (Item : Item_Ptr) return Byte_Index is
   begin
      if Item.Node = 0 then
         Raise_Exception
         (  Constraint_Error'Identity,
   ...
   function Get_Value (Item : Item_Ptr) return Byte_Index is
   begin
      if Item.Node = 0 then
         Raise_Exception
         (  Constraint_Error'Identity,
   ...
   begin
      if Parent = 0 then
         Insert ((Container.Self, 0, 1), Key, Value, Child);
      else
         declare
            Index : Integer :=
                    Find
   ...
   procedure Remove
      ...
      Lock      : Holder (Container.Pool);
   begin
      Remove (Container, Item.Node, Item.Index, Value);
      Item := No_Item;
   end Remove;
   ...
   begin
      Search (Container, Container.Root_Bucket, Key, Node, Index);
      if Index > 0 then
         Remove (Container, Node, Index, Value);
      end if;
   end Remove;

   procedure Remove (Container : in out B_Tree; Key : Byte_Index) is
      Replaced : Byte_Index;
      Node     : Byte_Index;
      Index    : Integer;
      Lock     : Holder (Container.Pool);
   ...
   begin
      if Item.Node = 0 then
         ...
         begin
            ...
            Replaced := Get_Value (Block, Item.Index);
            Set_Value (Block, Item.Index, Value);
         end;
      end if;
   end Replace;

   procedure Replace (Item : Item_Ptr; Value : Byte_Index) is
      Replaced : Byte_Index;
   begin
      Replace (Item, Value, Replaced);
   end Replace;
            ...
            if Index > 0 then
               declare
                  Block : Block_Type renames
                          Update (Pool.File, Node).all;
               begin
                  Replaced := Get_Value (Block, Index);
   ...
   procedure Replace
      ...
      Old : Byte_Index;
   begin
      Replace (Container, Key, Value, Old);
   end Replace;
   ...
   procedure Unchecked_Erase (Container : in out B_Tree) is
   begin
      Erase (Container, Container.Root_Bucket);
      Container.Root_Bucket := 0;
   end Unchecked_Erase;
   ...
   function Unchecked_Get_Value (Item : Item_Ptr) return Byte_Index is
   begin
      if Item.Node = 0 then
         Raise_Exception
         (  Constraint_Error'Identity,
   ...
   begin
      Remove (Item.Tree.all, Item.Node, Item.Index, Value);
   end Unchecked_Remove;
